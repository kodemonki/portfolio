...
export class Starfield extends Scene {
  ...
  create() {
    ...
    this.wisp = this.add.particles(1024 * 2, 768 * 2, "flares", {
      frame: "white",
  ...
  update() {
    if (this.wisp != undefined) {
      this.wisp.x = this.input.mousePointer.x;
      this.wisp.y = this.input.mousePointer.y;
      const x1 = this.input.mousePointer.x;
      const y1 = this.input.mousePointer.y;
      const x2 = 1024 * 2;
      const y2 = 768;
      let angleRadians = Math.atan2(y1 - y2, x1 - x2);
